	...
	else
	{
		...
		while (infile)
		{
			for (int i = 0; i < 1; i++)
			{
				...
				infile >> price;
				infile >> description;
				obj[i] = new Wind(gen_type, type, description, price);
				try
				{
					if (type == aux && gen_type == "Wind")
					{
						cout << "--------------------------------" << endl;
	...
	else
	{
		...
		while (!infile.eof())
		{
			for (int i = 0; i < 1; i++)
			{
				...
				infile >> description;
				infile >> stock_count;
				obj[i] = new Wind("Wind", type, description, price);
				try
				{
					if (type == aux)
					{
...
bool Wind::file_empty(std::ifstream& pFile)
{
